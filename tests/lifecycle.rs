...
use common::{
    ...
    probe::{
        DRAIN_DEADLINE, PHASE_LIMIT, SHELL_BODY, SHUTDOWN_TERM_LIMIT, read_frames_until,
...
#[test_case(vec![] ; "clean_close_without_terminal_frame")]
#[tokio::test(flavor = "multi_thread")]
async fn terminal_response_bails_client_at_143(respond: Vec<Message>) {
...
#[tokio::test(flavor = "multi_thread")]
async fn garbled_traffic_fails_as_transport_error_at_1() {
    ...
    let writer = tokio::spawn(async move {
        ...
        let _ = tokio::time::timeout(common::WAIT_LIMIT, released).await;
    });
    let mut client = common::Client::at(&socket).spawn(&["echo", "hi"]);
    let deadline = Instant::now() + common::WAIT_LIMIT;
    while client.try_wait().is_none() {
        assert!(
            Instant::now() < deadline,
            "the garbled client did not exit within {:?}",
            common::WAIT_LIMIT
        );
...
#[tokio::test(flavor = "multi_thread")]
async fn per_run_epoch_stamped_log_file_is_created_in_log_dir() {
    let server = Server::builder().start().await;
    let entries: Vec<String> = match fs::read_dir(server.path().join("logs")) {
        Ok(entries) => entries
            .map(|entry| {
                entry
...
#[tokio::test(flavor = "multi_thread")]
async fn live_socket_refuses_startup_naming_path_and_leaves_owner_untouched() {
    let server = Server::builder().start().await;
    ...
    let mut second_server = std::process::Command::new(common::bin_path("ncap-server"))
        .arg("--socket")
        .arg(&socket)
        .arg("--log-dir")
    ...
    let status = common::wait_bounded(&mut second_server, PHASE_LIMIT, "the second server");
...
#[tokio::test(flavor = "multi_thread")]
async fn stale_socket_file_is_removed_and_bind_succeeds() {
    let dir = tempfile::tempdir().expect("tempdir");
...
#[tokio::test(flavor = "multi_thread")]
async fn shutdown_signal_bails_client_at_143_and_removes_socket(signal: i32) {
    ...
    let client_status = client
        .wait_within(common::probe::BAIL_LIMIT, "the client to bail")
        .status;
...
#[tokio::test(flavor = "multi_thread")]
async fn connection_finishing_inside_grace_window_exits_cleanly_with_announcement() {
...
#[tokio::test(flavor = "multi_thread")]
async fn shutdown_signal_terms_whole_group_including_grandchildren() {
    let mut server = Server::builder().start().await;
...
#[tokio::test(flavor = "multi_thread")]
async fn connection_past_deadline_is_dropped_when_drain_expires() {
    let mut server = Server::builder()
        .timeout(DRAIN_DEADLINE.as_secs())
        .start()
        .await;
