...
| `0x02` | `Stdin` | client → server | raw bytes |
...
4. The server sends exactly one terminal frame — `Exit` on child completion, `Error` on spawn/decode failure — and nothing after it.
5. Either side may then close.

## Guarantees and conventions

- **Version is advisory.** Client and server ship from the same package and are always in lockstep; a mismatch (or a missing version) is a warning on the client's stderr / server's log, never a rejection.
- **Ordering:** per-stream FIFO is guaranteed; interleaving between stdout and stderr is not (independent forwarding tasks).
- **EOF:** there is no EOF frame. The client closing its write half ends stdin; the server drops the child's stdin pipe, giving the child EOF.
